...
SWEP.Purpose = "A baseball bat for beating the shit out of people or... playing baseball.";
SWEP.Contact = "";
				...
				if ( trace.Entity:IsPlayer() or trace.Entity:IsNPC() ) then
					...
					if ( IsValid(physicsObject) ) then
						...
						if (!player) then
							timer.Simple(FrameTime() * 0.5, function()
								if ( IsValid(trace.Entity) ) then
									trace.Entity:TakeDamageInfo( Clockwork.kernel:FakeDamageInfo( (self.Primary.Damage * 2), self, self.Owner, trace.HitPos, DMG_CLUB, 2 ) );
								end;
							end);
						else
							timer.Simple(FrameTime() * 0.5, function()
